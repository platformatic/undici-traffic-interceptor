...
import { createTrafficApp } from './traffic-app.ts'
...
async function runBenchmark () {
  console.log('Starting target and traffic apps...')
  const targetApp = await createTargetApp(TARGET_PORT)
  const traffic = await createTrafficApp(TRAFFIC_PORT)
  console.log('Apps started successfully')

  // Create agents
  const baseAgent = new Agent()
  const interceptorAgent = new Agent().compose(createTrafficInterceptor({
    labels: {
  ...
  // TODO wait for traffic to receive all the meta requests
